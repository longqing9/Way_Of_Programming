 ...
 */
@Controller
@RequestMapping(value = "${frontPath}/sys/role")
public class RoleControllerGson extends BaseControllerGson {
	...
	public String get(@RequestParam(required=false) String id,HttpServletResponse response) {
		...
		VoRole voRole=new VoRole();
		Role role=new Role();
		
		if (StringUtils.isNotBlank(id)){
			role=systemService.getRole(id);
			UserUtils.setVoRole(voRole, role);
			return JsonMapper.toJsonString(voRole);
		}else{
			return JsonMapper.toJsonString(voRole);
		}
	}
...
//	@RequiresPermissions("sys:role:view")
	...
	public String list(Role role, Model model,HttpServletResponse response) {
		response.setCharacterEncoding("UTF-8");
		response.setContentType("text/html;charset=utf-8");
		response.setHeader("Access-Control-Allow-Origin", "*");
		response.setHeader("Access-Control-Allow-Headers", "Access-Control");
		
		List<VoRole> voRoleList=new ArrayList<VoRole>();
		
		List<Role> list = systemService.findAllRole();
		
		for (Role role2 : list) {
			VoRole voRole=new VoRole();
			UserUtils.setVoRole(voRole,role2);
			System.out.println("role2:"+role2);
			voRoleList.add(voRole);
		}
		System.out.println("..........."+JsonMapper.toJsonString(voRoleList));
//		model.addAttribute("list", list);
		return JsonMapper.toJsonString(voRoleList);
	}
...
//	@RequiresPermissions("sys:role:edit")
	...
	public String save(Role role, Model model, RedirectAttributes redirectAttributes,HttpServletResponse response) {
	...
	public String delete(Role role, RedirectAttributes redirectAttributes,HttpServletResponse response) {
		if(!UserUtils.getUser().isAdmin() && role.getSysData().equals(Global.YES)){
			addMessage(redirectAttributes, "越权操作，只有超级管理员才能修改此数据！");
			return "redirect:" + adminPath + "/sys/role/?repage";
		}
...
//		}
	...
	public String assign(Role role, Model model,HttpServletResponse response) {
		List<User> userList = systemService.findUser(new User(new Role(role.getId())));
		model.addAttribute("userList", userList);
		return JsonMapper.toJsonString(userList);
	}
