...
<body>
		...
		<div id="TitleArea_Title">
			<div id="TitleArea_Title_Content">
				<img border="0" width="13" height="13"
					src="style/images/title_arrow.gif" /> 科目信息
			</div>
		</div>
	...
	<!-- 过滤条件 -->
	<div id="QueryArea">
		<form action="${pageContext.request.contextPath }/SelectTeacherBysubname.action" method="post">
	...
	<div id="MainArea">
		<table class="MainArea_Content" align="center" cellspacing="0" cellpadding="0">
			...
			<tbody id="TableData" align="center" >
				<c:forEach items="${list }" var="sutea" varStatus="index">
					<tr class="TableDetail1">
					<td>${index.count }</td>
					<td>${sutea.subname}</td>
					<td>${sutea.tname }</td>
					<td>
						<a href="${pageContext.request.contextPath}/ttinformation.action?tno=${sutea.tno}"  class="FunctionButton">教师信息</a>
					</td>
				</tr>
				</c:forEach>	
			</tbody>
		...
		<div class="paginate">
			<c:if test="${page.countpage!=1 }">
				<div class="FunctionButton"><a href="${pageContext.request.contextPath}/subject.action?pc=1">&nbsp;首页&nbsp;</a></div>
			</c:if>
			<c:if test="${page.currentpage>1 }">
				<div class="FunctionButton"><a href="${pageContext.request.contextPath}/subject.action?pc=${page.currentpage-1}">&nbsp;上一页&nbsp;</a></div>
			</c:if>
			<c:choose>
			...
			<c:forEach var="i" begin="${begin }" end="${end }">
				<c:choose>
					...
					<c:otherwise>
						<div class="FunctionButton"><a href="${pageContext.request.contextPath}/subject.action?pc=${i}">&nbsp;[${i }]&nbsp;</a></div>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<c:if test="${page.currentpage < page.countpage }">
				<div class="FunctionButton"><a href="${pageContext.request.contextPath}/subject.action?pc=${page.currentpage+1}">&nbsp;下一页&nbsp;</a></div>
			</c:if>
			<c:if test="${page.countpage!=1 }">
				<div class="FunctionButton"><a href="${pageContext.request.contextPath}/subject.action?pc=${page.countpage}">&nbsp;末页&nbsp;</a></div>
			</c:if>
