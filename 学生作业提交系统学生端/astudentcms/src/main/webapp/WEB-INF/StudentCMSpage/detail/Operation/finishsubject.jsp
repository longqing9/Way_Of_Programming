...
<body>
	...
	<div id="QueryArea">
		<form action="/wirelessplatform/cuisine.html" method="get">
			...
			<input type="text" name="keyword" title="请输入科目名称">
	...
	<div id="MainArea">
			...
			<tbody id="TableData" align="center" >
				<c:forEach items="${list }" var="list" varStatus="index">
					<tr class="TableDetail1">
						...
						<td>${list.subname }</td>
						<td>
							<a href="${pageContext.request.contextPath}/selectoinformation.action?opid=${list.opid }"  class="FunctionButton">进入查看</a>
							<c:if test="${list.rescore!=null && list.rescore!=0 }">
								<a target="blank" href="${pageContext.request.contextPath}/playvideo.action?opid=${list.opid }"  class="FunctionButton">讲解</a>
							</c:if>
							<c:if test="${list.rescore==null || list.rescore==0 }">
								<a href="${pageContext.request.contextPath}/deleteoperation.action?opid=${list.opid}"  class="FunctionButton" onclick="return delConfirm()">删除</a>
							</c:if>
						</td>
				</tr>
				</c:forEach>
		...
		<div class="paginate">
			<c:if test="${page.countpage!=1 }">
				<div class="FunctionButton"><a href="${page.url}&&url=${page.url }&&pc=1">&nbsp;首页&nbsp;</a></div>
			</c:if>
			<c:if test="${page.currentpage>1 }">
				<div class="FunctionButton"><a href="${page.url}&&url=${page.url }&&pc=${page.currentpage-1}">&nbsp;上一页&nbsp;</a></div>
			</c:if>
			<c:choose>
			...
			<c:forEach var="i" begin="${begin }" end="${end }">
				<c:choose>
					...
					<c:otherwise>
						<div class="FunctionButton"><a href="${page.url}&&url=${page.url }&&pc=${i}">&nbsp;[${i }]&nbsp;</a></div>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<c:if test="${page.currentpage < page.countpage }">
				<div class="FunctionButton"><a href="${page.url}&&url=${page.url }&&pc=${page.currentpage+1}">&nbsp;下一页&nbsp;</a></div>
			</c:if>
			<c:if test="${page.countpage!=1 }">
				<div class="FunctionButton"><a href="${page.url}&&url=${page.url }&&pc=${page.countpage}">&nbsp;末页&nbsp;</a></div>
			</c:if>
