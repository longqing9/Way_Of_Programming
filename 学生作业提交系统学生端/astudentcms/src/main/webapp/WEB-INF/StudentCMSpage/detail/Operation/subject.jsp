...
<body>
	...
	<div id="QueryArea"> 
		<%-- <form action="${pageContext.request.contextPath }/SelectoperationByoptop.action" method="post">
			...
			<input type="text" name="optop" title="请输入科目名称">
	...
	<div id="MainArea">
			...
			<tbody id="TableData" align="center" >
				<c:forEach items="${list }" var="list" varStatus="index">
					<tr class="TableDetail1">
						...
						<td>${list.tname }</td>
						<td>
							<a href="${pageContext.request.contextPath}/finishsubject.action?subid=${list.subid }&&url=${pageContext.request.contextPath}/finishsubject.action?subid=${list.subid }"  class="FunctionButton">进入查看</a>
						</td>
					</tr>
				</c:forEach>
			</tbody>
		...
		<div class="paginate">
			<c:if test="${page.countpage!=1 }">
				<div class="FunctionButton"><a href="${pageContext.request.contextPath}/showsubject.action?pc=1">&nbsp;首页&nbsp;</a></div>
			</c:if>
			<c:if test="${page.currentpage>1 }">
				<div class="FunctionButton"><a href="${pageContext.request.contextPath}/showsubject.action?pc=${page.currentpage-1}">&nbsp;上一页&nbsp;</a></div>
			</c:if>
			<c:choose>
			...
			<c:forEach var="i" begin="${begin }" end="${end }">
				<c:choose>
					...
					<c:otherwise>
						<div class="FunctionButton"><a href="${pageContext.request.contextPath}/showsubject.action?pc=${i}">&nbsp;[${i }]&nbsp;</a></div>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<c:if test="${page.currentpage < page.countpage }">
				<div class="FunctionButton"><a href="${pageContext.request.contextPath}/showsubject.action?pc=${page.currentpage+1}">&nbsp;下一页&nbsp;</a></div>
			</c:if>
			<c:if test="${page.countpage!=1 }">
				<div class="FunctionButton"><a href="${pageContext.request.contextPath}/showsubject.action?pc=${page.countpage}">&nbsp;末页&nbsp;</a></div>
			</c:if>
